...
| **WF-3** | [MUST] | A Quotation is an estimate only. It creates no charge on the customer ledger, never allots an invoice number, and carries a validity date. | Issue a quotation. No entry appears in the customer ledger, the invoice register or the receipt register. |
...
*It is NOT an Invoice.* It creates no receivable and no entry on the customer ledger, and is an **estimate** that the final invoice may differ from because the actual sample count or weight differed.
...
*It is NOT an accounting asset record.* This system keeps a **working asset record** — supplier, purchase document, cost, date put to use, useful life, physical verification, condemnation reference — alongside the **metrological control register** (range, interval, calibration result, correction factors, out-of-service status). What it does not keep is the financial fixed-asset register: capitalisation, depreciation and book value stay with the unit's accounts wing and the parent institute, outside this software. Both records exist; they are not the same register.
...
- **FY** — the Indian financial year, 1 April to 31 March, written **`2026-27`** in full and compressed to **`2627`** where character length is constrained. Only six series are length-constrained — invoice, bill of supply, advance receipt voucher, refund voucher, credit note and debit note. Every other series writes the year in full, including in worked examples.
...
| **Proforma / advance demand number** | `DVM/PF/2026-27/<4>` | `DVM/PF/2026-27/0123` | Financial year | Front Desk, Accounts, and the customer | Not a tax document, so no length limit. A demand only: it creates no charge on the customer ledger and no entry in the invoice register. |
...
| **Debit Note number** | `DVM/D/<FY4>/<4>` | `DVM/D/2627/0006` | Financial year | Accounts, Unit Incharge, and the customer | **Gap-free and statutory.** Must reference the invoice number and date it adjusts. Raised only where the taxable value or the tax charged on an issued invoice was **less** than what is payable. |
...
| **WF-99** | [MUST] | The following series are **gap-free**: invoice, bill of supply, advance receipt voucher, refund voucher, credit note, debit note, money receipt, report (per type), ULR, complaint and nonconformity. Gaps are permitted in enquiry, quotation, worksheet, conditioning job, sample, sub-sample and allocation series. | Run the Series Register report for a gap-free series. Every number from 1 to the current counter is listed as Issued or Cancelled, and no number is reported Missing. |
