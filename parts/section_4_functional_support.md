...
**What this module is for, in plain words.** Every instrument in the laboratory that affects a test result must be under control. That means the laboratory knows what the instrument is, where it is, whether it is working, when it was last calibrated, when the next calibration is due, and which tests were done on it. The draft note called this an "asset system". It is actually two registers that happen to describe the same physical objects: a **working asset record** — what the instrument is, what it cost, who supplied it, where it stands and whether it is still fit to hold — and a **measurement control register** for calibration and traceability. This module builds both and keeps them linked. It does not build an accounting asset register: capitalisation, depreciation and book value stay with the unit's accounts wing and the parent institute, outside this software (N4, M22.1). The single most valuable function in the whole module is the **out-of-calibration impact analysis**: when a balance or a tensile tester is found to be wrong, the system must instantly list every result it produced since it was last known to be good, so the laboratory can decide whether any issued report has to be withdrawn.
...
| Working Asset Record tab | Equipment Custodian, Accounts (read) | What the instrument cost, who supplied it, on what document, when it was put to use, its expected life, and its physical verification and condemnation history — printable as an asset card the accounts wing can use |
...
| M11-28 | [SHOULD] | The system shall hold a **working asset record** for each instrument — what it is, what it cost, where it came from and whether it is still fit to hold: purchase indent or order reference, supplier, supplier's invoice number and date, purchase cost, date put to use, warranty start and end, funding source or scheme, asset classification, expected useful life, physical verification date and outcome, condemnation order reference, disposal date and disposal value. These are facts copied from documents the laboratory holds, and calibration planning, replacement planning and traceability need them. The system shall **not** compute or store depreciation method, depreciation rate, accumulated depreciation or book value: depreciation accounting and the financial fixed-asset register stay with the unit's accounts wing and the parent institute, outside this software (N4, M22.1). The record shall print as an asset card the accounts wing can use, and shall be exportable through the dormant interface in M22 should an accounting system ever be adopted. | The Working Asset Record tab prints an asset card containing every listed field, and the same data is available as an export file. No depreciation, accumulated-depreciation or book-value field exists anywhere in the interface or the schema. |
...
Working asset record tab (see M11-28): purchase indent or order reference, supplier, supplier's invoice number, invoice date, purchase cost, date put to use, warranty start, warranty end, funding source or scheme name, asset class, expected useful life (years), last physical verification date, verification outcome, condemnation order reference, disposal date, disposal value. There is deliberately no depreciation method, depreciation rate, accumulated depreciation or book-value field: those belong to the financial fixed-asset register, which stays with the unit's accounts wing and the parent institute and is outside this system's scope (N4, M22.1).
...
| Purchase Indent | Store Keeper, Lab In-Charge | Raise a request that goes to the unit's purchasing process |
...
| M12-20 | [MUST] | The system shall raise a **purchase indent** listing items, quantities, required-by date, technical acceptance criteria per item, suggested supplier and estimated value, and route it for Lab In-Charge approval. On approval it shall **print for hand-over to the unit's accounts wing**, which runs the procurement chain outside this software (N3). The system shall record the hand-over — date, the officer it was given to, and a scan of the receipted copy where one is returned — and shall allow the Store Keeper to enter, against that indent, the **sanction or purchase order reference** the accounts wing supplies, so that a goods receipt has a purchase document to name. An indent shall carry a state — Draft, Approved, Handed over, Ordered (reference recorded), Received, Closed, Cancelled — and shall stay on an open-indent worklist until it is closed, so that a request nobody acted on is visible rather than forgotten. Where an external accounting system is ever configured under M22 the approved indent may additionally be transmitted to it; that path is switched off, and is neither built nor tested at go-live (M22-17, OPEN-Q35). | Approve an indent; it prints, the hand-over is recorded with a date and a named officer, and it appears on the open-indent worklist. Enter the purchase order reference the accounts wing returns; receiving a lot against that indent offers the reference automatically. Close the indent; it leaves the worklist. No step in the test requires an external system. |
...
| M12-33 | [MUST] | The system shall record, for every purchased lot, **what was bought, from whom, on what document and at what price**: the supplier, the supplier's invoice or bill number and date, the purchase rate per unit and the resulting lot value, and shall retain the scanned purchase document with the lot alongside the certificate of analysis. These are facts copied from documents the laboratory holds. The system shall **not** compute a costing method, a stock revaluation or any ledger posting; those stay with the unit's accounts wing, which keeps the books. | Receive a lot against a supplier's bill; the Consumable Lot Register and the lot card show the supplier, the bill number and date, the rate and the value, with the scanned bill attached; searching that bill number finds the lot and, through M12-12, every test that used it. |
...
| Linked to external accounting item | Reference | No | Populated by the M22 interface where it is enabled; empty otherwise |
...
| Supplier's invoice / bill number | Text | Yes where purchased | The commercial document the material arrived on. A lot received without purchase records the source in Remarks under M12.6 rule 7 |
| Supplier's invoice / bill date | Date | Yes where purchased | |
| Purchase rate per unit | Amount | Yes where purchased | As billed, exclusive of tax |
| Lot value as received | Amount | System-computed | Quantity received x purchase rate. A stores figure copied from the supplier's bill, not an accounting valuation: no costing method, revaluation or ledger posting is computed anywhere in this system |
| Purchase document file | File upload | Yes where purchased | The scanned bill or cash memo, held with the lot alongside the certificate of analysis |
...
9. **Stock value, and how far it goes.** No external accounting system exists, so nothing outside this software holds a figure for what is in store. The LIMS therefore records, per lot, the price it was bought at and the document it was bought on (M12-33), and the stock position report of M20-29 shows quantity and that recorded value side by side. It goes no further: the system runs **no costing method, no stock revaluation and no ledger posting**. Those follow from the movements and stay with the unit's accounts wing, which keeps the books. If an accounting system is ever adopted it becomes the system of record for money value, and the LIMS remains the system of record for **fitness for use** — quarantine, acceptance, expiry, blocking. The two must not both decide whether a lot may be used. See M22.
...
**OPEN-Q6:** Does any consumables inventory exist outside this system — in the accounts wing's records or on paper — and does it track lot and expiry? — *Recommended default:* the LIMS holds the register in full, including lot, expiry, certificate and fitness-for-use; if an external accounting system is ever adopted, send it only quantity and value movements and do not attempt to store expiry there.
...
**Recommendation on ownership, stated up front.** The unit runs no external accounting system, so the LIMS owns all of the following: the tax invoice and its number series, the receipt, the credit note, the tax masters and the customer ledger, along with the **commercial decision and the link to the work** — which rate applies, what the charge is, which test each charge line belongs to, whether the report may be released, and the reconciliation of money against orders. The general ledger is not among them: the unit's accounts wing keeps the books, and this module feeds them. The split is still worth stating, because it may not stay this way. If the parent body ever puts an accounting package in place, that package should own the **accounting ledger** — the invoice and the receipt as posted accounting documents, the credit note, the tax masters and the general ledger — and the LIMS should keep the commercial decision, for a simple reason: only the LIMS knows which test a rupee belongs to, and only a ledger system can post to a ledger. The field-by-field table is in M22, written down and switched off.
...
| M17-33a | [MUST] | The system shall record every **refund of money paid out** as a **Refund Payment** in its own gap-free financial-year series, carrying: the credit note (M17-35) or refund voucher (M17-33) it discharges, as a hard link and not a free-text note; the customer; the amount; the outward mode from a controlled list (cash at the counter; demand draft; bank transfer; reversal to the instrument the money arrived on; adjustment against the customer's advance balance); the outward reference values with their types, dates and evidence files, on the same reference table as M17-14; the **sanction reference and the sanctioning officer, both mandatory**, the officer being the Approving Authority where the amount exceeds `waiver_approval_limit`; and the date the money actually left. The originating Payment moves to status *Refunded* only when a Refund Payment is committed against it, never by editing the status. A Refund Payment is money **out**: it shall never be counted as a collection in the Daily Collection Register (M17-24) or in the collection-versus-remittance reconciliation (M17-25), shall appear on both as a separate outward total, and shall post to the customer ledger (M17-21) as a debit on the date the money left. No money shall be refunded except against a committed credit note or refund voucher, and no Refund Payment shall be deleted (M17-26 applies unchanged). | Approve a credit note with a refund and attempt to pay it with the sanction reference blank; refused. Record it with the sanction reference; the originating payment reads *Refunded*, the day's collection total is unchanged, the refund shows in its own outward total, and the customer ledger carries the debit. Attempt to set a payment to *Refunded* directly; refused. |
...
| Customer | Link | Yes | |
| Amount | Money | Yes | |
| Mode | List | Yes | The inward list of M17-13 where Direction is In; the outward list of M17-33a where Direction is Out |
| Direction | In / Out | Yes | Default In; Out only on a Refund Payment (M17-33a) |
| Credit note or refund voucher discharged | Link | Conditional | Required where Direction is Out; M17-33a permits no refund without one |
| Refund of payment | Link | Conditional | Required where Direction is Out; the payment being refunded |
| Sanction reference and sanctioning officer | Text plus link to staff | Conditional | Mandatory where Direction is Out |
...
| Applied to orders | Child table: order, amount applied | Yes unless held as an advance or Direction is Out | A Refund Payment discharges a credit note or refund voucher, not an order |
...
| M20-06 | [MUST] | The system shall produce a **Reagent and Consumable Register**: item, batch, manufacturer, supplier, supplier's invoice or bill number and date, receipt date, expiry date, quantity received, quantity remaining, purchase rate per unit and lot value as recorded under M12-33, storage location, certificate on file yes or no, and state. | Print; every batch appears with its state, and a purchased batch carries the bill it arrived on, its rate and its recorded value. |
...
| M20-29 | [MUST] | **Stock position and consumption.** Current quantity by item and batch, the purchase rate and the lot value as recorded under M12-33 shown alongside the quantity, items below reorder level, batches expiring within the alert windows, and consumption per test and per month. The value column is the price the material was bought at, not a valuation: the report applies no costing method and posts to no ledger (M12.6 rule 9). | Run it; the position reconciles to the stock ledger, and each lot shows its recorded rate and value beside its quantity. |
...
| Refund payment — record / sanction | – | – | – | C | A | – | C (raise) | – | – | R own |
...
| `external_accounting_interface_enabled` | **Off**, off at delivery, and off is the only state the system is built, tested and validated in | M22-17, M22-01, M22-19, M12-20, M12.3 item master, M21-72a. While Off, no outbound accounting message is produced, no external reference is required on any screen (M22-19), the external-reference fields on the customer, item, invoice and asset records stay empty and hand-editable, and nothing in M22 outside M22-17, M22-18 and M22-20 is present in the running system. The switch exists so that an interface can be commissioned later without a schema change. Turning it On requires OPEN-Q35 to have been answered with a named system and the ownership table of M22.1 to have been agreed in writing first, and is a change under M21-57 change control. |
...
| M21-72a | [MUST] | The LIMS masters customer legal name and address, so a pseudonymisation decision shall be applied to the master record itself and shall not be reversible through any routine screen. Where the optional interface in M22 is ever enabled and an external system masters the name, the decision shall additionally be held as a **LIMS-side suppression flag that inbound reconciliation honours**, and shall raise a request to that system to pseudonymise its own copy; until it confirms, the customer shall be shown as pseudonymised in the LIMS and the pending request shall appear on an exception list. | Pseudonymise a customer; the name is gone from the master and no screen restores it. With the M22 interface enabled, run the nightly reconciliation and confirm the name is not restored and the pending external request is listed. |
...
## M22. Interface to an External Accounting System

**What this module is for, in plain words.** The laboratory runs no external accounting system. There is no package holding its customers, its invoices, its receipts or its stock — the LIMS owns all of that itself, which is what M22-17 requires, and it keeps its own working record of the equipment it holds. This module therefore specifies machinery that is **written down and switched off**: the field-by-field ownership rules, the message plumbing and the reconciliation controls that would apply if the parent body ever mandated an accounting system for the unit. It is written now, while the shape of the data is fresh, because retro-fitting an ownership rule to a system that has been running for two years costs far more than stating it once at the start. Read every "ERP" below as "whatever external accounting system may one day exist": no product has been chosen and none is named here. The governing rule is stated once and applied everywhere: **for every field, exactly one system is the master, and the other holds a read-only copy.** The commonest integration failure in this kind of project is two editable copies of a customer's address.

**Stated plainly: there is nothing to integrate with today.** Everything below is therefore a design held in reserve, with the assumption written next to it, and a list of things that must be learned about whatever system is eventually named before any of it is built. Only M22-17, M22-18 and M22-20 are in scope for go-live, for the reasons given under M22.2.

### M22.1 Ownership table

**Read this table as conditional.** No external accounting system exists, so nothing in the left-hand column is owned today by software this project exchanges data with. The LIMS holds what it needs to run: the customer master, the tax invoice and its number series, receipts, credit notes, the tax masters, the customer ledger, consumable stock, and its own working asset record. What lies outside this system's scope is not held here in any configuration and stays on the unit's own books — the general ledger, the purchase chain beyond the laboratory's own indent, and the financial asset register with its capitalised cost and depreciation. The left-hand column therefore records where mastery would move if an accounting system were ever adopted, and nothing more; the right-hand column records what the LIMS would go on mastering even then.

| Data or function | Owned by the external accounting system | Owned by LIMS | Direction of sync | Trigger | Failure handling |
|---|---|---|---|---|---|
...
| **New customer created at the laboratory counter** | Master, eventually | **Master today; creates the definitive customer record** | LIMS → ERP, then ERP → LIMS to confirm | On acceptance of the first order | The customer record works fully in the LIMS with no external identifier at all; the identifier is filled in when the push succeeds; **sample receipt is never blocked**. This is **not** the `is_provisional` record of M1-13, which means something else entirely — a walk-in whose own details are not yet complete — and the two must never be conflated in the schema or on screen |
...
| **Invoice number series** | Master, if such a system is ever adopted and already maintains a compliant financial-year series | **Master today, and by default** | One or the other, never both | – | **This must be decided once and never split.** See OPEN-Q36 |
...
**When these apply.** No external accounting system exists, so most of this table is built only if and when the parent body names such a system and the interface is commissioned; the acceptance checks are run then. Three requirements are exceptions and bind from day one. **M22-17** describes how the laboratory actually operates. **M22-18** governs the unpaid-balance hold, and under M17-21 that balance comes from the LIMS's own customer ledger, so the rule that a commercial hold must never quietly change a technical decision applies now and not later. **M22-20** requires the frozen issued files to be stored by the LIMS with their checksums, and M21-72 rests on it: pseudonymisation is permitted only because those files are guaranteed to survive it unaltered. **How to read the priority column below.** Except for M22-17, M22-18 and M22-20, every priority in this table is the priority that requirement would carry **once an interface is commissioned**. None of the others is a go-live obligation; none may be counted into a phase scope in §26, into the validation pack of M21.9, or into the acceptance run of §27, until OPEN-Q35 has been answered with a named system. Read them as [MUST-WHEN-COMMISSIONED].

| ID | Priority | Requirement | Acceptance check |
|---|---|---|---|
| M22-01 | [MUST] | The integration shall be built as a set of **calls over a web interface** (representational state transfer over secure transport) using an **outbox pattern** with **idempotency keys**. *Outbox pattern, in two sentences: when the LIMS needs to tell the external system something, it first writes the message into its own outbox table in the same database transaction as the business change, so the message can never be lost even if the network is down; a separate background worker then reads the outbox and delivers the messages, retrying until each one succeeds.* | Cause a delivery failure; the business change is still committed, the message stays in the outbox, and it is delivered when the connection returns. |
...
| M22-14 | [MUST] | Before any upgrade of the external system goes live in production, the integration shall be tested against the upgraded version in staging, and the result recorded in the Change Control Register of M21-57. **This binds this project's side only, and it depends on notice that nobody has yet undertaken to give (OPEN-Q40): if an interface is ever commissioned, a named contact and a written commitment to give notice before an upgrade shall be obtained first and held with the interface's configuration record. Without that commitment the interface is run on the assumption that it may break without warning, and the reconciliation report of M22-08 is the detector.** | The change register holds an entry for the upgrade naming the version tested against and the outcome, and the interface configuration record names the contact who gave the notice. |
| M22-15 | [SHOULD] | Where the external system offers no programmatic interface, the integration shall fall back to **file exchange**: the LIMS writes a file to an agreed folder in an agreed format, the ERP imports it, and the ERP writes a response file the LIMS reads. The same outbox, idempotency key, logging and reconciliation requirements shall apply. | Configure file mode; the same reconciliation report works. |
| M22-16 | [SHOULD] | The LIMS is a separate application. It is not a module inside anyone's accounting package, and there is no package for it to sit inside; this requirement is therefore a standing constraint on any later proposal to fold it into one, and it applies to that proposal whenever one is made. Were the LIMS ever built as a module, the ownership table in M22.1 shall still be honoured field by field, and the technical records, audit trail, frozen report files and the enforcement gates in M11, M12, M13, M15 and M16 shall remain under the LIMS's own control and shall not be reachable by the host package's generic editing screens. | Attempt to edit a verified result through a generic data screen of the host package; refused. |
| M22-17 | [MUST] | **This is how the laboratory operates.** The LIMS shall run with the integration switched off entirely, using its own customer master, its own invoice numbering and its own stock records, and shall be complete and compliant in that state. The integration is a feature that may be added later, never a foundation: no requirement anywhere in this document shall depend on an external accounting system existing. | Install the system with no integration configured at all; every module in this document works end to end, invoicing and stock included. Switch a test interface on later; the accumulated data pushes across without hand back-filling. |
| M22-18 | [MUST] | A **commercial hold shall never silently influence a technical decision.** Where an unpaid balance is reported — today by the LIMS's own customer ledger under M17-21, and in future by any external accounting system — that shall appear as an explicit, logged commercial hold on report release, separate from and visible alongside the technical release state. This one binds from day one, because the balance is the laboratory's own record. | Put a customer's ledger into unpaid balance; verification of results still proceeds; only the report release is held, visibly and with a reason. |
| M22-19 | [SHOULD] | The system shall provide a **manual entry field for every reference an external system would otherwise supply** — the external asset number, the external purchase-order number, the external customer identifier and the external system's own document reference — so that a value can be typed in, and **no screen shall require any such reference before it will save**. This does **not** extend to the tax invoice number, the bill of supply number, the advance receipt voucher number, the refund voucher number, the credit note number or the money receipt number: those are allotted by this system's own locked counter under WF-97, WF-98 and DB-08, are gap-free under WF-99, and are **never typed in by a user** — today, or after any interface is ever commissioned. | Save each record carrying an external reference with that reference left blank; accepted. Then look for a way to type the invoice number: there is no such field, and the number shown is the one the counter allotted. |
...
### M22.3 Assumptions any future integration would rest on

**These are not facts about an existing product.** No accounting system has been named for this unit, so none of the following is known. This is the checklist to run against whatever system is eventually mandated, before a line of integration code is written; each row says what has to change in the design if the answer turns out to be no.

| # | Assumption | What breaks if it is wrong |
|---|---|---|
| 1 | The system holds a customer master with at least name, address, tax registration number, state and a stable identifier | The LIMS goes on mastering the customer entirely, as it does today, and the external system becomes a downstream copy — a bigger change to the ownership table than it looks |
| 2 | The system can create an invoice through an interface, or can import one from a file | Invoicing has to be done twice, once in each system, and the reconciliation report becomes the only control |
| 3 | The system maintains, or can be made to maintain, a financial-year invoice series that is consecutive, at most sixteen characters, and unique in the year | The LIMS goes on mastering invoice numbering, as it does today, and the external system must accept the LIMS's number; if neither can, the laboratory has a compliance problem that is not a software problem |
| 4 | The system can hold custom fields, or can at least store an external reference against a customer, an item, an invoice and an asset | Every link has to be maintained in a mapping table inside the LIMS, which works but adds a reconciliation burden |
| 5 | The system's inventory does not enforce expiry or quarantine | If it does, the two systems will fight over whether a batch may be used; the rule must then be that the LIMS decides and the ERP is told |
| 6 | The system has a fixed-asset register with depreciation | If not — and today there is none — the asset fields in M11-28 are the laboratory's only working record of them and must be printable as an asset card |
| 7 | The LIMS is a separate application that would integrate with such a system, not a module inside it. This row is a decision, not an assumption | If it were ever made a module, M22-16 applies and the enforcement gates must be protected from the host package's generic editing |
...
| 9 | The system honours an idempotency key supplied by the caller, so a retried message cannot create a second invoice, receipt or indent on its side | M22-02's acceptance check cannot be run at all. Deduplication moves to this side — hold the message, query the far side for the record, send only if it is absent — and the reconciliation report of M22-08 becomes the only guard against a duplicate invoice in what is, on our side, a gap-free statutory series |
| 10 | The system can be **read** as well as written: its receipts, its customer records, its item master and its stock balances can be listed through the same interface | M22-08 shrinks to a one-way check — it can list what this system sent and never got an acknowledgement for, and nothing else. Every other mismatch then surfaces only when a person compares two printouts |
| 11 | The system tolerates a message carrying a schema version, and does not fail on a version it does not recognise | M22-13 cannot be honoured from this side alone. The transition period becomes a scheduled cut-over — both systems changed on the same day with the interface stopped in between — and M22-14's test becomes mandatory rather than prudent |
...
5. **The laboratory never adopts an external accounting system, or adopts one and later drops it.** The first is the present position and the LIMS is built for it; the second it must survive. This is the practical reason for M22-17: the integration is a feature, not a foundation.
...
**OPEN-Q35:** Will the parent body ever mandate an accounting system for this unit, and if so what is its integration surface — a documented web interface, direct database access, file import and export, or nothing? What authentication does it use? — *Recommended default:* assume none is coming, build and run standalone under M22-17, and answer the rest of this question only when a system is actually named. **Nothing in M22 beyond M22-17, M22-18 and M22-20 is to be built before that answer exists.**

**OPEN-Q36 — ANSWERED.** *Question was:* which system will master the tax invoice number series? The LIMS does, because it is the only system there is, and its series must therefore satisfy the consecutive, sixteen-character, financial-year-unique rule of M17-28 in its own right. The question reopens only if an accounting system is one day adopted. — *Recommended default:* keep the series in the LIMS. Should such a system arrive, let it master the number if its own series is already compliant, because the accounting document belongs there, and otherwise let it record the LIMS's number as supplied. Decide once. Never split.

**OPEN-Q37:** Could a future accounting system hold custom fields or an external reference against customer, item, invoice and asset records? — *Recommended default:* assume not, and design the mapping to live inside the LIMS with the reconciliation report as the control. This costs nothing today, because the mapping table is empty.

**OPEN-Q38 — ANSWERED.** *Question was:* is the LIMS to be a module inside another package, or a separate application? It is a separate, standalone application. There is no package for it to sit inside, and the reasons would hold even if there were: the offline requirement, the immutable audit trail, the frozen report files and the enforcement gates are all easier to guarantee in software the laboratory controls, and standalone keeps the LIMS alive whatever the parent body's accounting arrangements turn out to be. — *Recommended default:* build standalone, as ARC-14 requires, and treat any later proposal to fold the LIMS into a package as a change governed by M22-16.

**OPEN-Q39:** Does any consumables inventory or fixed-asset register exist for this unit outside this system — in the accounts wing's records, or on paper? — *Recommended default:* assume paper at most; load the LIMS registers from scratch and treat them as the working record. If an accounting system later arrives holding either, agree the ownership table with whoever maintains it before pushing anything across.

**OPEN-Q40:** If an accounting system is ever named, who maintains it, and will they commit to a stable interface and to notifying this project before an upgrade? — *Recommended default:* the LIMS runs with the integration switched off (M22-17), so nothing is blocked on this. Should the day come, obtain a named contact and a written note of the interface version in use before integration work starts, and treat the integration as its own phase with its own approval.