...
| **OPEN-Q-D26** | OPEN-Q-D26: Who pays for the short message service, and is there an existing parent-institute arrangement (the national testing portal and mobile application already send messages) that this unit can use? — | ask CSTRI whether the national portal's messaging account can be extended to the unit; if not, budget a small prepaid volume and keep email plus the printed counter acknowledgement as the guaranteed channels, with short message service as an enhancement rather than a dependency. |
