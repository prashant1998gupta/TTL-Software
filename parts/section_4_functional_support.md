...
| Asset / Depreciation tab | Accounts | Fields the parent institute's asset register and the unit's accounts wing need |
...
| M11-28 | [SHOULD] | The system shall hold **asset and depreciation fields** for the parent institute's asset register: purchase order reference, supplier, invoice number and date, capitalised cost, date put to use, funding source or scheme, asset classification, expected useful life, depreciation method and rate, accumulated depreciation, book value, physical verification date and outcome, condemnation reference. The unit runs no external accounting system, so these fields are the laboratory's own **working record** of what it holds, kept so that an asset card can be printed and handed to the accounts wing; the parent institute's financial asset register, and depreciation accounting itself, stay where they are. The fields shall also be exportable through the dormant interface in M22 should such a system ever be adopted. | The Asset tab prints an asset card containing all listed fields; the same data is available as an export file. |
...
| M12-20 | [MUST] | The system shall raise a **purchase indent** listing items, quantities, required-by date, technical acceptance criteria per item, suggested supplier and estimated value, route it for Lab In-Charge approval, and print it for the unit's own purchasing process. Where an external accounting system has been configured under M22, the approved indent shall also be transmitted to it, and shall queue and retry while that system is unreachable. | Approve an indent; it prints. With the M22 interface enabled, it also appears in the outbound queue and, on success, carries the external purchase-requisition number back. |
...
9. **Where an external accounting system holds stock valuation.** None exists at present, so the LIMS carries both the quantity and the value of what is in store, which is what a stock register needs; the accounting entries that follow from those movements stay with the unit's accounts wing, which keeps the books. If such a system is ever adopted it becomes the system of record for money value, and the LIMS remains the system of record for **fitness for use** — quarantine, acceptance, expiry, blocking. The two must not both decide whether a lot may be used. See M22.
...
| M22-19 | [SHOULD] | The system shall provide a **manual override entry** for every ERP-supplied reference — invoice number, receipt number, asset number, purchase order number — so that a value can be typed in. With no external system in place this is the only path and it shall be a first-class one, not a fallback: no screen shall require an externally supplied reference before it will save. | Type an external invoice number manually; the record links correctly. Save each affected record with the reference left blank; accepted. |
